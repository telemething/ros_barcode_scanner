...
image_draw::image_draw(const NodeOptions& options)
:   Node("image_draw", options)
{
    on_shutdown([&]
    {
        image_draw::shutdown_callback();
    });
...
{
    return CV_RGB(255 * color_msg.r, 255 * color_msg.g, 255 * color_msg.b);
}
...
void image_draw::image_callback(const sensor_msgs::msg::Image::ConstSharedPtr& image_msg) 
{
    ...
    Time time_now = this->get_clock()->now();

    // While iterating over the ImageMarkers, the expired ImageMarkers must be removed.
    for(auto& map_key_value_pair : m_marker_map)
    {
        Time expire_time(map_key_value_pair.second.second, time_now.get_clock_type());

        if(expire_time <= time_now)
        {
            m_marker_map.erase(map_key_value_pair.first);
        }
        else
        {
...
void image_draw::image_marker_callback(const marker_t::SharedPtr msg)
{
    ...
    if(msg->action == marker_t::ADD)
    {
        ...
        {
            expire_time = Time(std::numeric_limits<int>::max(), 0);
        }
...
void image_draw::shutdown_callback()
{
